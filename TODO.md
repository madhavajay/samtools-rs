...
- **Other complex algorithms still TODO** (samtools-rs-only but each is a multi-hundred-LOC implementation): `depad` (623 LOC C), full `checksum` (1324 LOC C; default SAM/BAM checksum path is partial), full `reference` (598 LOC C; SAM/BAM MD path is partial), `cram-size` (blocked on htslib-rs CRAM internals), `markdup` full stats parity, `fastq` exact name-grouped paired/singleton/other routing, `import -0` with paired inputs, and `sort` external/template-coordinate/minimiser sorts.
...
- [~] `fixmate` (`bam_mate.c`) — basic mate flag/pos fixup for adjacent paired records in name-sorted BAM and SAM inputs (`FMUNMAP`, `FMATE_REVERSE_COMPLEMENTED`, `mate_reference_sequence_id`, `mate_alignment_start`). Default MC/MQ mate aux tags are added for mapped mates and cleared when the mate is unmapped. `-m` adds `ms:i` mate-score tags for markdup. `-z`/`--sanitize` now parses and validates through the shared sanitizer option parser. Default `@PG` insertion via `pg::add_samtools_pg_to_header` and `--no-PG` are supported. `-r` removes secondary and unmapped alignments and clears `PROPER_PAIR`/`MATE_REVERSE` on the surviving mate when its partner is unmapped, matching upstream's `remove_reads` semantics. **Pending:** record-level sanitizer mutation, `-c` mode, CRAM, mate-rescore.
- [~] `markdup` (`bam_markdup.c`, 89k) — single-end and paired-end duplicate marking for SAM and BAM inputs. SE records are keyed by `(tid, pos, reverse-flag)` plus optional barcode tag; PE records pair by qname and are keyed by the canonical pair of `(tid, pos, strand)` triples plus optional per-end barcode tag. Within a group the entry with highest (combined) MAPQ stays primary and the rest receive `BAM_FDUP`; secondary and supplementary alignments with duplicate primary qnames inherit the duplicate flag and are removed by `-r`. Supports `-b TAG`/`--barcode-tag TAG`, `-c` (clear existing duplicate flags and duplicate metadata tags), `-S` (accepted; propagation is always on), `-t` duplicate-origin `do` tags, `-d DISTANCE` optical-distance duplicate classification with `dt:Z:SQ`/`dt:Z:LB` tags, default QCFAIL exclusion with `--include-fails` override, optical-aware estimated library size in `-s` stats, `-r` (remove duplicates from output), `-s` (upstream-shaped summary counts to stderr), `-O sam|bam`, `-o FILE`, default `@PG` insertion, and `--no-PG`. **Pending:** exact upstream stats output/count parity, CRAM.
