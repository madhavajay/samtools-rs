...
**Phases 0–2 complete; Wave A complete (with partials); Wave B substantially complete (with partials); Wave D substantially complete (with partials); Wave C in progress (`fixmate`, `rmdup`, `calmd`, `reset`, `markdup` partials landed); Phases 4–5 pending. Remaining work largely blocks on htslib-rs infrastructure (pileup APIs, CRAM all-record iterator, custom-index paths) or on substantial per-subcommand efforts (full `mpileup`/`consensus`/`phase`/`ampliconstats`/`targetcut`, `depad`, full `checksum`, full `reference`, `markdup` optical-distance/full stats, `fastq` exact paired/singleton/other semantics, `sort` external merge / template-coordinate / minimiser sorts).**

Subcommands shipped (29 of ~40):
...
- 🟡 partial implementation: `view` (SAM↔SAM, SAM→BAM/CRAM, count/header, region queries, `-f/-F/-G/-q` filters, `-L` BED, `-e` filter expression, `-x/--keep-tag` aux strip, `-p`/`-U` for SAM-input binary output, SAM-output `@PG`/`--no-PG`), `cat` (SAM/BAM record-level concat, `-h`, `-r region` for indexed BAM, `@PG`/`--no-PG`), `reheader` (SAM/BAM with `-c` filter, `@PG`/`--no-PG`), `fastq`/`fasta`/`bam2fq`, `split` (with `--no-PG`, `--write-index`), `sort` (in-memory coordinate/name/tag + `@PG`/`--no-PG`), `merge` (in-memory + `-R region`/`-L BED` + `@PG`/`--no-PG`), `collate` (in-memory + `@PG`/`--no-PG`), `import`, `rmdup` (single-end + paired-end + `@PG`/`--no-PG`), `markdup` (single-end + paired-end + barcode key + secondary/supplementary qname propagation + `-r`/`-s`/`-O`/`-o`/`@PG`/`--no-PG`), `bedcov` (CIGAR-walk), `coverage` (CIGAR-walk + ASCII histogram), `depth`, `addreplacerg` (SAM/BAM `-O sam|bam`, `overwrite_all` default, `@PG`/`--no-PG`), `fixmate` (name-sorted BAM/SAM, MC/MQ, `-r` mode, `@PG`/`--no-PG`), `reset` (alignment field clear, default aux strip, `--reject-PG`/`--no-RG`/`--no-PG` matching upstream `noPGentry` semantics, `@PG` insertion), `stats` (extensive SN coverage plus `-f`/`-F` flag filters, `-l` read-length filtering, FFQ/LFQ quality histograms, GCF/GCL GC histograms, and approximate COV coverage histogram), `calmd`/`fillmd` (SAM/BAM/reference-backed CRAM text MD/NM + SAM BAQ paths + `-d` + `@PG`/`--no-PG`), `reference` (SAM/BAM MD-tag reconstruction + `-r`/`-o`/`-q`)
...
- **Other complex algorithms still TODO** (samtools-rs-only but each is a multi-hundred-LOC implementation): `depad` (623 LOC C), full `checksum` (1324 LOC C; default SAM/BAM checksum path is partial), full `reference` (598 LOC C; SAM/BAM MD path is partial), `cram-size` (blocked on htslib-rs CRAM internals), `markdup` optical-distance/full stats, `fastq` exact name-grouped paired/singleton/other routing, `import -0` with paired inputs, and `sort` external/template-coordinate/minimiser sorts.
...
Rust tests: 354 currently passing. `cargo fmt --all --check`, `cargo clippy -p samtools-rs --all-targets -- -D warnings`, and `cargo test -p samtools-rs` are green after the most recent additions: shared `@PG` insertion via `pg::add_samtools_pg_to_header` integrated into `sort`, `merge`, `collate`, `addreplacerg`, `reset`, `fixmate`, `rmdup`, `calmd`, and `view` SAM-output paths; `view -p`/`-U` for SAM-input BAM and CRAM output via text-roundtrip; `merge -R region` and `-L BED` indexed-BAM restrictions; `cat` gained SAM record-level concatenation and `-r region` indexed-BAM restriction; `depth -H` header output, `-f` input file lists, flag filters (`-g`, `-G`/`--excl-flags`, `--incl-flags`, `--require-flags`), and `-l` minimum read length filtering; SAM/BAM `reheader` with command-filtered headers; `coverage -m`/`-A`/`-w` ASCII histogram output, `-b`/`--bam-list`, `--ff`/`--excl-flags`, `--rf`/`--incl-flags`, `-l` minimum read length filtering, and `-d` maximum-depth capping; `bedcov -g`/`-G` filter-mask controls and `-j` deletion/refskip skipping; `fixmate -r` mode plus default MC/MQ mate tags; `rmdup` gained paired-end duplicate removal; SE + PE `markdup` (qname-paired groups, combined MAPQ score, barcode-key grouping with `-b`/`--barcode-tag`, secondary/supplementary qname propagation, `-r`/`-s`/`-O`/`-o`/`@PG`/`--no-PG`); `calmd` gained SAM/BAM/reference-backed CRAM text MD/NM recomputation against FASTA references and `-d` BQ-tag removal; `stats` extended with `-f`/`--required-flag`, `-F`/`--filtering-flag`, and `-l`/`--read-length`, `is sorted`, supplementary, insert size mean/stddev, inward/outward/other oriented pair counts, total/average/maximum sequence length (per fragment-1/fragment-2), bases mapped (incl. cigar), mismatches, error rate, bases duplicated, bases trimmed, average quality, FFQ/LFQ quality histograms, GCF/GCL GC histograms, approximate COV coverage histograms with `-c`/`--coverage` bin ranges, `-g`/`--cov-threshold` target-percentage SN lines, and percentage properly paired; `checksum` gained SAM/BAM default output plus `-P`/`-C`/`-M` columns, `-B`, wildcard scalar/string/array aux tags, `-a` field-selection shorthand, and report merging; `reference` gained SAM/BAM MD-tag reconstruction with `-r`/`-o`/`-q`; `sort` gained in-memory `-t TAG` ordering with coordinate/name secondary keys and upstream-style `SS`; `reset --no-PG` semantics fixed to match upstream (preserve existing `@PG`, skip new entry); `addreplacerg` gained SAM/BAM `-O sam|bam` record-path RG header/tag rewriting, and its default mode changed to upstream's `overwrite_all`.
...
- [~] `sort` (`bam_sort.c`, 138k — the largest single file) — basic in-memory coordinate sort, `-n` name sort, and `-t TAG` tag sort for BAM and SAM inputs. Supports `-o`, `-O sam|bam`, `--output-fmt sam|bam`, coordinate-sort BAM `--write-index`, default `@PG` insertion via `pg::add_samtools_pg_to_header`, `--no-PG`, and sets `@HD SO`/tag-sort `SS`. **Pending:** on-disk external merge for large inputs, template-coordinate sort (`-M`), minimiser sort (`-N`), thread/memory caps, CRAM.
- [~] `merge` (`bam_sort.c` shared) — basic in-memory multi-input merge for BAM and SAM inputs with coordinate or name sort. `-f` overwrite, `-o`, `-O sam|bam`, `--output-fmt sam|bam`, coordinate-sort BAM `--write-index`, default `@PG` insertion via `pg::add_samtools_pg_to_header`, `--no-PG`, `-R region` (indexed BAM only), and `-L bed` (indexed BAM only, de-duplicates records from overlapping BED intervals) are supported. **Pending:** k-way streaming merge, header merging for inputs with differing `@SQ`, CRAM.
- [~] `collate` / `bamshuf` (`bamshuf.c`) — basic in-memory name-sort grouping for BAM and SAM inputs. `-o PREFIX`, `-O` stdout, validated `--output-fmt sam|bam`, default `@PG` insertion via `pg::add_samtools_pg_to_header`, and `--no-PG` are supported. **Pending:** on-disk hash-bucket implementation for inputs larger than memory, `-r` random seed, `-n` in-memory record cap, CRAM.
- [~] `cat` (`bam_cat.c`) — basic SAM and BAM concatenation works (record-level decompress + re-encode). Supports `-o`, `-h` (header replacement), default `@PG` insertion, `--no-PG`, and `-r region` (indexed BAM only — restricts each input to records overlapping the region via `query_bam_records_from_path`). **Pending:** BGZF block-level fast path, CRAM, `-p N/M`.
...
- [~] `fixmate` (`bam_mate.c`) — basic mate flag/pos fixup for adjacent paired records in name-sorted BAM and SAM inputs (`FMUNMAP`, `FMATE_REVERSE_COMPLEMENTED`, `mate_reference_sequence_id`, `mate_alignment_start`). Default MC/MQ mate aux tags are added for mapped mates and cleared when the mate is unmapped. `-z`/`--sanitize` now parses and validates through the shared sanitizer option parser. Default `@PG` insertion via `pg::add_samtools_pg_to_header` and `--no-PG` are supported. `-r` removes secondary and unmapped alignments and clears `PROPER_PAIR`/`MATE_REVERSE` on the surviving mate when its partner is unmapped, matching upstream's `remove_reads` semantics. **Pending:** record-level sanitizer mutation, `-c`/`-m` modes, CRAM, mate-rescore.
- [~] `markdup` (`bam_markdup.c`, 89k) — single-end and paired-end duplicate marking for SAM and BAM inputs. SE records are keyed by `(tid, pos, reverse-flag)` plus optional barcode tag; PE records pair by qname and are keyed by the canonical pair of `(tid, pos, strand)` triples plus optional per-end barcode tag. Within a group the entry with highest (combined) MAPQ stays primary and the rest receive `BAM_FDUP`; secondary and supplementary alignments with duplicate primary qnames inherit the duplicate flag and are removed by `-r`. Supports `-b TAG`/`--barcode-tag TAG`, `-r` (remove duplicates from output), `-s` (basic counts to stderr), `-O sam|bam`, `-o FILE`, default `@PG` insertion, and `--no-PG`. **Pending:** optical-duplicate distance (`-d`), full upstream stats output, CRAM.
...
- [~] `stats` (`stats.c` + `stats_isize.c`, 123k + 8k) — `SN` (Summary Numbers) section plus FFQ/LFQ first/last fragment quality histograms, GCF/GCL first/last fragment GC histograms, and approximate CIGAR-walk COV coverage histograms for SAM, BAM, and reference-backed CRAM region paths, including `-f`/`--required-flag`, `-F`/`--filtering-flag`, `-l`/`--read-length`, `-c`/`--coverage MIN,MAX,STEP` COV binning, and `-g`/`--cov-threshold` target percentage lines. SAM and BAM iterate records directly to populate sequence-length, quality, GC, CIGAR, NM, and COV accumulators (CRAM non-region remains on the `summarize_*` summary path and therefore reports zeros for those fields). The emitted lines now cover: raw total / filtered / sequences / is sorted (header SO) / 1st & last fragments / mapped / mapped+paired / unmapped / properly paired / paired / duplicated / MQ0 / QC-failed / non-primary / supplementary / total length / total first & last fragment length / bases mapped / bases mapped (cigar) / mismatches (NM aux) / error rate / average length & per-fragment / maximum length & per-fragment / average quality / singletons / insert size mean & stddev / inward, outward, and other oriented pair counts / pairs on different chromosomes / percentage of properly paired reads / target bases / target genome coverage above threshold. SAM, indexed BAM, and reference-backed CRAM positional region arguments and `-t` target files restrict the summary and COV positions, with overlapping BAM/CRAM regions de-duplicated. `-d` / `--remove-dups` filters duplicate-marked primary records and their quality/GC/COV histogram contributions. Missing CRAM references fail cleanly. **Pending:** exact pileup-backed COV byte parity, per-cycle, BAQ, runtime sortedness check (currently relies on the header `SO` tag), CRAM non-region read-length filtering, and CRAM non-region sequence-length/quality/GC/COV stats (requires a CRAM all-record iterator in `htslib-rs`).
...
- [~] `reference` (`reference.c`) — SAM/BAM MD-tag reconstruction to FASTA is implemented, including `-o`, `-q`, and basic `-r region` output. **Pending:** embedded-reference CRAM mode (`-e`, blocked on CRAM container/block internals), CRAM input MD path (needs CRAM all-record iteration/reference handling), indexed region iteration for large files, and full upstream `test_reference` parity.
