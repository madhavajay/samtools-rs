...
- [~] `fixmate` (`bam_mate.c`) — basic mate flag/pos fixup for adjacent paired records in name-sorted BAM and SAM inputs (`FMUNMAP`, `FMATE_REVERSE_COMPLEMENTED`, `mate_reference_sequence_id`, `mate_alignment_start`) and rejects `@HD SO:coordinate` input like upstream. Default MC/MQ mate aux tags are added for mapped mates and cleared when the mate is unmapped. `-m` adds `ms:i` mate-score tags for markdup. `-c` adds lowercase template-CIGAR `ct:Z` tags to the earlier mapped mate and clears stale `ct` tags from both mates. `-z`/`--sanitize` now parses and validates through the shared sanitizer option parser. Default `@PG` insertion via `pg::add_samtools_pg_to_header` and `--no-PG` are supported. `-r` removes secondary and unmapped alignments and clears `PROPER_PAIR`/`MATE_REVERSE` on the surviving mate when its partner is unmapped, matching upstream's `remove_reads` semantics. **Pending:** record-level sanitizer mutation, CRAM, mate-rescore.
