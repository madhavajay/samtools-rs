...
Rust tests: 200 currently passing (quickcheck:6, flags:3, dict:1, view:35, head:5, sort_merge:4, misc:56, stats_wave_d:28, test_status:1, library/command unit tests:61) — `cargo test -p samtools-rs` green; `cargo fmt --check` and `cargo clippy -p samtools-rs --all-targets -- -D warnings` also clean as of the latest full run.
...
- samtools-rs: https://github.com/madhavajay/samtools-rs/pull/2
...
- [~] `view` (`sam_view.c`, 68k) — partial: SAM↔SAM passthrough, SAM→BAM/CRAM, SAM/BAM/reference-backed CRAM stdin count/text/BAM/CRAM paths, reference-backed CRAM→SAM text/count paths including flag/MAPQ filtered count mode, reference-backed CRAM→BAM full-file and region output, reference-backed BAM→CRAM and CRAM→CRAM full-file and region output, header-only / count modes (including CRAM `-H`), `-h` `-H` `-c` `-b` `-C` `-T` `-o` `--no-PG`, filter flags `-f`/`-F`/`-G`/`-q` for SAM output/count modes and SAM-input BAM/CRAM output, region queries (`<chr:start-end>`), `-L FILE` BED restrict, `-U FILE` unselected SAM-output splitting for flag/MAPQ filters, `-p/--unmap` SAM-output marking for records failing flag/MAPQ filters (sets UNMAP, MAPQ=0, CIGAR=`*`, TLEN=0), `-e EXPR` filter expression count/SAM/BAM/CRAM output modes for SAM/BAM/reference-backed CRAM (including indexed BAM/CRAM regions and SAM/BAM/reference-backed CRAM stdin), `-O FORMAT` output-fmt option. Unsupported BAM/CRAM-input binary-output `-f/-F/-G/-q` combinations now fail loudly instead of silently writing unfiltered records. **Pending:** aux-tag manipulation, BAM/CRAM-input binary flag/MAPQ filters, binary and expression-aware `-U`/`-p` output, multi-file inputs, paired-aware filters, full CRAM parity.
- [x] `head` (`sam_view.c` shared) — SAM and BAM input; CRAM header-only modes; reference-backed CRAM record extraction for `-n N`; `-h N`, `-n N`, all-default. stdin still TODO.
...
- [~] `depth` (`bam2depth.c`) — per-position depth via CIGAR walks for BAM and reference-backed indexed CRAM. `-a`/`-aa`/`-d`/`-q`/`-o`, `-r` region restriction, and `-b` BED restriction are supported. Tests cover successful CRAM region depth with top-level `--reference` and clean missing-reference failure. **Pending:** pileup-based exact handling of overlaps/deletions, multi-input columnar output, CRAM input without an explicit reference.
- [~] `coverage` (`coverage.c`) — per-reference/`-r` region `numreads`, `covbases`, `coverage`, `meandepth`, `meanmapq` via CIGAR walks for BAM and reference-backed indexed CRAM. Tests cover successful CRAM region coverage with top-level `--reference` and clean missing-reference failure. **Pending:** `meanbaseq` (needs per-base qualities), histogram/ASCII-plot output modes, CRAM input without an explicit reference.
...
- [~] `stats` (`stats.c` + `stats_isize.c`, 123k + 8k) — basic `SN` (Summary Numbers) section for SAM, BAM, and reference-backed CRAM: raw total / filtered / 1st & last fragments / mapped / paired / properly paired / unmapped / duplicated / MQ0 / QC-failed / non-primary / singletons / diffchr pairs. Missing CRAM references fail cleanly. **Pending:** FFQ/LFQ, IS (insert sizes), GCF, COV histogram, per-cycle, BAQ, region restriction, CRAM input without an explicit reference.
