...
- **Other complex algorithms still TODO** (samtools-rs-only but each is a multi-hundred-LOC implementation): `depad` (623 LOC C), full `checksum` (1324 LOC C; default SAM/BAM checksum path is partial), full `reference` (598 LOC C; SAM/BAM MD path is partial), `cram-size` (blocked on htslib-rs CRAM internals), `markdup` full stats/library-size parity, `fastq` exact name-grouped paired/singleton/other routing, `import -0` with paired inputs, and `sort` external/template-coordinate/minimiser sorts.
