...
- [~] `collate` / `bamshuf` (`bamshuf.c`) — in-memory grouping for BAM, SAM (tolerant `c/C/s/S/I` aux reader), and reference-backed CRAM. **Non-fast order is the exact upstream `bamshuf` order**: bucket by `hash_X31_Wang(qname) % 64`, then sort each bucket by `(hash, qname, flag>>6&3)` (ported `hash_Wang` bit-mix). SAM output emits the **raw input header** with `@HD SO:unsorted GO:query` applied (preserving input `@RG`/`@SQ` field order) + records via `sam_render`. Output format inferred from the `-o` filename extension when `--output-fmt` is absent. `-f` fast mode (primary READ1/READ2 pairs early, `-r` working-read cap), `-o`/`-O`/`-n`/positional prefix/`--no-PG`. **Byte-exact vs upstream collate `{-O, collate1, collate2, collate3, 1_fast_collate}`** (5/6); test `collate_*` in `sort_merge.rs`. **Pending:** the fast-mode `-r` small-store deferred order (`2_fast_collate_with_tmp`), on-disk hash-bucket for >memory inputs, CRAM output.
...
- [~] `consensus` (`bam_consensus.c`, 126k, + `consensus_pileup.c`) — **`--mode simple`** implemented on the htslib-rs pileup engine: `calculate_consensus_simple` freq/score weighting, `-c`/`--call-fract`, `-H`/`--het-fract`, `-d`/`--min-depth`, `-q`/`--use-qual`, `-A`/`--ambig`, `--min-BQ`, `--show-del`/`--show-ins` (insertion sub-columns synthesized from `PileupRead`), FASTA/FASTQ/pileup output, IUPAC het table, `100*used_score/tscore` quality (capped 93 for FASTQ), `-o`, multi-line wrap. **Byte-exact vs every upstream `test/consensus/expected` fixture defined in `consensus.reg`** (`1-4.out`, `1q-3q.out`, `1p.out`, `5q/5p.out` incl. the `--call-fract 0.600/0.601` boundary). Test `consensus_simple_matches_upstream_fixtures`. **Pending:** default `recall`/Bayesian modes (`calculate_consensus_gap5m`), `-r region`, `-aa` all-positions padding, multi-input.
...
- [x] **Indexing BAMs without `@HD SO:coordinate` metadata** — **DONE** (TODO-NEXT #6, htslib-rs `530b27c`). `index_compat::build_bai` builds via the manual record loop (like the CSI/SAM-BAI builders) instead of `noodles_bam::fs::index`, so coordinate-ordered BAMs whose header omits the SO tag (`test/dat/test_input_1_{a,b}.bam`) index like upstream; `range.bam.bai` byte regression preserved. `samtools index` works on them; test `index_bam_without_so_coordinate_header`.
