...
- [~] `addreplacerg` (`bam_addrprg.c`) — SAM/BAM add/replace for the `@RG` header line and `RG:Z` aux tag. `-r SPEC`, `-R ID`, default-to-first-header-`@RG`-ID when neither is given (matching upstream; errors only if the input has no `@RG` line), `-m overwrite_all|orphan_only` (default `overwrite_all`), upstream `@RG` header reconciliation (`-r` + `overwrite_all` strips other `@RG` lines via the `sam_hdr_remove_except` equivalent; `-w` overwrites an existing same-ID `@RG` instead of erroring), `-O sam|bam`, `-o FILE`, default `@PG` insertion, and `--no-PG`. The SAM→SAM path remains streaming text rewrite; other SAM/BAM combinations use mutable `RecordBuf` records. The upstream `addrprg/{1,2,4}_fixup*` fixtures match (ignoring `@PG`). **Pending:** CRAM input/output, mate-aware updates, full orphan-first semantics.
