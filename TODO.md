...
- The full upstream `repos/samtools/test/test.pl` passes, bgzip-honest:
...
0. **[DONE 2026-05-18] Trust reset — local gate honesty.** The local parity
   ...
   - [x] Document the requirement (`README`/dev setup): prebuilt at
     `repos/htslib-rs/repos/htslib/{bgzip,tabix}` via
     `make -C repos/htslib-rs/repos/htslib tabix bgzip`; export that dir on `PATH`.
...
3. ~~**Remove the advisory full-harness escape hatch.**~~ **Done.** The full
   `repos/samtools/test/test.pl` run is now a required CI gate; `test_bgzip` is
   satisfied by the external htslib `bgzip` tool on `PATH`.

## Current Inputs

- `repos/samtools/`: upstream C samtools source and test suite. 54 C files (~42k LOC), ~40 subcommands dispatched from `bamtk.c:227`, and a 4159-line Perl test harness (`repos/samtools/test/test.pl`) with expected-output fixtures under `repos/samtools/test/<subcommand>/`.
- `repos/htslib-rs/`: sibling pure-Rust HTSlib compatibility workspace. Re-exports `noodles` and ships HTSlib-shaped adapters under `crates/htslib-rs/src/*_compat.rs`. Currently has 41 passing HTSlib test groups.
...
- **Tests — two gates**:
  1. **Parity gate**: upstream `repos/samtools/test/test.pl` run against the Rust binary. Expected outputs are the checked-in files under `repos/samtools/test/`. Used as a regression gate in CI.
...
- **C oracle**: local dev only. Devs MAY build upstream `samtools` (in `repos/samtools/`) and use `test.pl --redo-outputs` to refresh expected fixtures. CI does NOT build or run C samtools — it only diffs against the checked-in fixtures.
...
- Examples under `repos/samtools/examples/`.
...
- [x] Create root `Cargo.toml` workspace mirroring `repos/htslib-rs/Cargo.toml`:
  - members: `crates/samtools-rs`, `crates/samtools-rs-cli`
  - workspace deps include `htslib-rs = { path = "repos/htslib-rs/crates/htslib-rs" }`
...
- [x] Add a top-level GitHub Actions workflow with two jobs:
  ...
  - Parity gate: build release binary, then run `cd repos/samtools/test && perl test.pl -e samtools=$WORKSPACE/target/release/samtools`.
...
- [x] **Version + feature string** (`samtools-rs/src/version.rs`): exports `SAMTOOLS_VERSION` constant (currently pinned to `1.23.1`, the upstream tag tracked by the `repos/samtools/` submodule). `long_version` prints both samtools and htslib-rs versions.
...
Each subcommand below maps to: (a) one Rust module under `crates/samtools-rs/src/commands/`, (b) `test_<name>` in `repos/samtools/test/test.pl` passing against the Rust binary, (c) at least one Rust integration test under `crates/samtools-rs/tests/<name>.rs`.
...
- [x] Pin `repos/samtools/` to a specific upstream release tag once Phase 0 lands (record tag + commit in `README.md` and `version.rs`). Current pin: upstream tag `1.23.1`, commit `6efb9b6da35224cf804921dedecf9fb8f411365d`.
- [x] Pin `repos/htslib-rs/` to a known-green commit when Phase 0 lands. Current pin: `61e6e72f14f251e0849e8fe87a420eff374892af` (merged all 12 completed library/infra blockers, with vendored noodles PR #6 merged; prior: `f61801c`, `3fadffc`, `8372873`, `530b27c`, `ca812dd`, `9cf30b3`, `e25f392`, `5b25622`, `da4d331`, `6bd6fb0`, `88bd29f`).
...
- `repos/samtools/` — upstream C source + tests, used as fixture and reference only.
- `repos/htslib-rs/` — sibling Rust HTSlib compatibility workspace consumed via path dep.
...
cd repos/samtools && perl test/test.pl
