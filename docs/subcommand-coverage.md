...
- **samtools-rs status:** 🟡 — basic adjacent name-sorted mate flag/reference/position fixup works for BAM and SAM inputs, including default MC/MQ mate tags, `-m` mate-score tags, `-c` template-CIGAR `ct` tags, and `-r`; mate rescore, sanitizer mutation, and CRAM remain.
