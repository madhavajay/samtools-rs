...
- **samtools-rs status:** 🟡 — single-end and paired-end duplicate marking for SAM/BAM exists with optional barcode-key grouping (`-b`/`--barcode-tag`), duplicate-flag clearing (`-c`), `-S` compatibility for supplementary propagation, secondary/supplementary qname propagation, `-r`, upstream-shaped `-s` summary fields, `-O`, `-o`, default `@PG`, and `--no-PG`; optical duplicate distance, exact stats parity, and CRAM remain.
