...
- **samtools-rs status:** 🟡 — in-memory coordinate, query-name, and `-t TAG` merge works for BAM and SAM inputs, including differing `@SQ` union/remap, compatible `@RG`/`@PG` union, `@CO` comment preservation, stdout `-`, accepted `-s`, `--output-fmt=FORMAT`, and `-R region` / `-L BED` restriction for indexed BAM; streaming k-way merge, broader header reconciliation beyond `@SQ`/`@RG`/`@PG`/`@CO`, and CRAM remain.
