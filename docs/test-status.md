...
| `test_collate` | partial | In-memory name grouping exists for BAM, SAM, and reference-backed CRAM inputs with default `@PG` insertion, upstream-style `@HD SO:unsorted GO:query`, `--no-PG`, and `-f` fast primary-pair mode with `-r` working-read cap; hash-bucket/on-disk mode, `-n` temporary-file count, and CRAM output remain. |
