...
| `test_fixmate` | partial | Basic adjacent name-sorted BAM and SAM mate fixup exists with default MC/MQ tags and `-m` mate-score tags; `-r` removes secondary/unmapped alignments and clears `PROPER_PAIR`/`MATE_REVERSE` on the surviving mate when its partner is unmapped. `-c`, mate rescore, and CRAM remain. |
