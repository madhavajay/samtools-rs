...
| `test_view` | partial | SAM/BAM/reference-backed CRAM basics, stdin paths, count/header, filters, BED, region queries, and expression filtering are covered by `crates/samtools-rs/tests/view.rs`. BAM/CRAM-input binary aux mutation, binary `-U`/`-p`, multi-input, paired filters, and full CRAM parity remain. |
...
| `test_cat` | partial | Record-level BAM concat exists with default `@PG` insertion and `--no-PG`; BGZF fast path, CRAM, `-p`, and `-r` remain. |
...
| `test_stats` | partial | Basic `SN` summary exists for SAM, BAM, reference-backed CRAM, SAM/indexed BAM/reference-backed CRAM positional regions, and SAM/indexed BAM/reference-backed CRAM `-t` target files, with overlapping BAM/CRAM regions de-duplicated. `-d` / `--remove-dups` filters duplicate-marked primary records from the basic summary. Histograms, insert size, GC, coverage, per-cycle, BAQ, and CRAM without explicit reference remain. |
| `test_depth` | partial | CIGAR-walk BAM/reference-backed CRAM depth exists, including `-r`, `-b`, `-a`/`-aa`, `-d`, `-q`, `-o`, and multi-input depth columns. Exact pileup overlap/deletion behavior and CRAM without explicit reference remain. |
| `test_merge` | partial | In-memory merge exists for BAM and SAM inputs; streaming merge, header reconciliation, region restriction, CRAM, and parity details remain. |
| `test_sort` | partial | In-memory coordinate/name sort exists for BAM and SAM inputs; external merge, tag/template/minimiser sorts, memory/thread caps, write-index, and CRAM remain. |
| `test_collate` | partial | In-memory name grouping exists for BAM and SAM inputs; hash-bucket/on-disk mode, random seed, record cap, and CRAM remain. |
| `test_fixmate` | partial | Basic adjacent name-sorted BAM and SAM mate fixup exists; MC/MQ tags, `-r`/`-c`/`-m`, mate rescore, and CRAM remain. |
...
| `test_reset` | partial | BAM and SAM reset exist for alignment fields/default aux strip; reverse-strand re-reversal, `--reject-PG`, `--dupflag`, and CRAM remain. |
| `test_checksum` | not-yet-ported | `checksum` is a stub. |
| `test_coverage` | partial | BAM/reference-backed CRAM CIGAR-walk coverage exists, including `-r`, `--min-depth`, `-Q`/`--min-BQ`, read map-quality filtering, multi-input aggregate rows, mean depth, mean base quality, and mean map quality. Histogram/ASCII plot output and CRAM without explicit reference remain. |
