...
| `test_fixmate` | partial | Basic adjacent name-sorted BAM and SAM mate fixup exists with default MC/MQ tags, `-m` mate-score tags, and `-c` template-CIGAR `ct` tags; `-r` removes secondary/unmapped alignments and clears `PROPER_PAIR`/`MATE_REVERSE` on the surviving mate when its partner is unmapped. Mate rescore, sanitizer mutation, and CRAM remain. |
