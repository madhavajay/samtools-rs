...
| `test_collate` | partial | In-memory name grouping exists for BAM, SAM, and reference-backed CRAM inputs with default `@PG` insertion and `--no-PG`; hash-bucket/on-disk mode, random seed, record cap, and CRAM output remain. |
