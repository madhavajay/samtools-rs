...
| `test_addrprg` | partial | SAM/BAM add/replace exists with `-O sam|bam`, default mode matching upstream (`overwrite_all`), default-to-first-header-`@RG`-ID when neither `-r`/`-R` is given, upstream `@RG` header semantics (`-r` + `overwrite_all` removes other `@RG` lines; `-w` allows overwriting an existing same-ID line, otherwise it errors), default `@PG` insertion, and `--no-PG`. The upstream `addrprg/{1,2,4}_fixup*.sam.expected` fixtures match (modulo `@PG`, which the harness ignores). CRAM, mate-aware behavior, and full orphan-first semantics remain. |
